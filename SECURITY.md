...
Secrets belong in Render environment settings. The Windows operator helper stores password-encrypted Ethereum V3 keystores using ethers and verifies both signers after reloading the file in a separate process. Passwords use hidden prompts and stdin; decrypted keys exist in process memory and, for deployment, its temporary environment. Explicit copy actions put keys on the clipboard for direct transfer to Render; clear it afterward. Back up the encrypted file and keep its password separately. Legacy Windows-encrypted files remain untouched, but may be unreadable outside their original encryption context. Do not commit secrets or use real-money wallets. Rotation is not automatic: changing a deployment or signing identity against the same archive fails startup.
